...
public class FileUtils {
	...
	public static ProdFileVO parseFileInfo(MultipartFile file,
		...
		String prodOriginFileNm = file.getOriginalFilename();
		...
		String prodFileNm = nowDateStr + "_" + uuid.toString() + "_" + prodOriginFileNm;

		String prodFilePath = attachPath;

		//이미지인지 다른 파일형태인지 검사
		File checkFile = new File(prodOriginFileNm);
		//업로드한 파일의 형식 가져옴(이미지파일들은 image/jpg, image/png ...)
		String type = Files.probeContentType(checkFile.toPath());

		if(type != null) {
			if(type.startsWith("image")) {
				prodFile.setProdFileCate("img");
			} else {
				prodFile.setProdFileCate("etc");
			}
		} else {
			prodFile.setProdFileCate("etc");
		}

		prodFile.setProdFileNm(prodFileNm);
		prodFile.setProdOriginFileNm(prodOriginFileNm);
		prodFile.setProdFilePath(prodFilePath);

		//실제 파일 업로드
		File uploadFile = new File(attachPath + prodFileNm);
